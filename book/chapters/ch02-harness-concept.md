...
이 한 번의 요청 뒤에서 하네스 메타 스킬(`/harness`)이 수행하는 과정은 다음과 같다:

1. **도메인 분석**: 사용자의 요청에서 핵심 작업 유형을 식별한다
2. **팀 아키텍처 설계**: 작업을 전문 영역으로 분해하고, 적절한 아키텍처 패턴을 선택한다
3. **에이전트 생성**: 각 전문 영역에 맞는 에이전트를 정의한다 (`.claude/agents/` 에 파일 생성)
4. **스킬 생성**: 각 에이전트가 사용할 작업 절차를 만든다 (`.claude/skills/` 에 파일 생성)
5. **오케스트레이터 생성**: 팀 전체를 조율하는 상위 스킬을 만든다
6. **검증**: frontmatter, 트리거 키워드, 에이전트 간 참조 일관성을 확인한다
