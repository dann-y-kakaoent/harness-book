...
하지만 **실용적 상한선**이 있다. Claude Code에서 SubAgent를 동시에 너무 많이 호출하면 컨텍스트 관리가 복잡해지고, 팬인 단계에서 분석가가 처리해야 할 정보량이 과도해진다.
...
### 심화: SubAgent 호출과 팬아웃 구현

Claude Code에서 팬아웃을 구현하는 메커니즘은 **SubAgent 호출**이다. 오케스트레이터가 여러 에이전트를 "동시에" 호출할 때, Claude Code의 내장 SubAgent 기능을 사용한다.
...
### Phase 2: 관점별 조사 (팬아웃)
- **에이전트**: `researcher` (관점 수만큼 병렬 호출)
- **실행 방식**: 각 관점에 대해 SubAgent를 생성하여 병렬 실행
- **SubAgent별 입력**:
  - SubAgent 1: `research/brief.md`의 관점 1 섹션
  - SubAgent 2: `research/brief.md`의 관점 2 섹션
  - SubAgent 3: `research/brief.md`의 관점 3 섹션
- **완료 조건**: 모든 SubAgent가 결과 파일을 생성할 때까지 대기
```

여기서 중요한 점은, Claude Code의 SubAgent는 **파일 시스템을 통해 통신**한다는 것이다. SubAgent 간에 직접 메시지를 주고받는 것이 아니라, 각 SubAgent가 지정된 경로에 파일을 생성하고, 다음 Phase의 에이전트가 그 파일을 읽는 방식이다.
...
팬아웃에서는 일부 SubAgent가 실패할 수 있다. 리서처 B가 웹 검색 결과를 제대로 수집하지 못했다면? 두 가지 전략이 있다:
